...
** Last update Tue May 19 19:13:14 2015 
*/
#include <parser.h>
...
t_ast		*parse_instr(t_ast **ast, t_lex **lexem)
{
  ...
  if ((next = lexer_next_token(*lexem)) && next->type != LX_EOL)
    {
      if (!(i_ast = parse_pipeline(ast, lexem)))
	return (NULL);
      while ((next = lexer_next_token(*lexem))
	     && next->type >= LX_AND && next->type <= LX_OR)
	{
	  tmp = i_ast;
	  ...
	  i_ast->left = tmp;
	  *lexem = next->next;
	  if (!(i_ast->right = parse_pipeline(ast, lexem)))
	    return (NULL);
	}
    }
  else if ((*lexem)->type != LX_EOL)
    return (parse_pipeline(ast, lexem));
  return (i_ast);
}
